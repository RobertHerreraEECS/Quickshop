...
class FontListViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        /*
        var temp = WalmartAPISearch(Query: sender.text)
        var temp2 = temp.returnURLBasedSearchQuery()
        UserInput = temp2
        println("Search Query URL: \(temp2)")
        ...
        DataManager.getSearchQueryDataFromWalMartWithSuccess { (ItemQueryData) -> Void in
            ...
            if let queryName = json["query"].string {
                println(" Your Search: \(queryName)\n")
            }
            
            if let totalResults = json["totalResults"].int {
                if totalResults == 0 {
                    println("\n\n Sorry, No results. Item may not exist\n")
                }
            }
